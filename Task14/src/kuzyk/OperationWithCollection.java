...
public class OperationWithCollection {
    ...
    public Map<String, Integer> frequency(LinkedList<String> arls) {
        ...
        numberOfMatches.stream().sorted().collect(Collectors.toList());
        System.out.println(numberOfMatches);

        for (int i = numberOfMatches.size() - 1; (i < numberOfMatches.size()) && (i > numberOfMatches.size() - 3); i--) {
    ...
    public void lengthWord(LinkedList<String> arls) {
        ...
        lengthOfWord.stream().sorted().collect(Collectors.toList());
        System.out.println(lengthOfWord);

        int max = listOfLength.getLast();

        for (int i = lengthOfWord.size() - 1; i >= lengthOfWord.size()-3; i--) {
